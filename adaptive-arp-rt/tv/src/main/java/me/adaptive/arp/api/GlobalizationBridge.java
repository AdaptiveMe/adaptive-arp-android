...
public class GlobalizationBridge extends BaseApplicationBridge implements IGlobalization, APIBridge {
    ...
    public String getResourceLiteral(String key, Locale locale) {
        ...
        if (logger != null)
            logger.log(ILoggingLogLevel.DEBUG, this.apiGroup.name(), this.getClass().getSimpleName() + " executing getResourceLiteral({" + key + "},{" + locale + "}).");
    ...
    public KeyPair[] getResourceLiterals(Locale locale) {
        ...
        if (logger != null)
            logger.log(ILoggingLogLevel.DEBUG, this.apiGroup.name(), this.getClass().getSimpleName() + " executing getResourceLiterals({" + locale + "}).");
    ...
    public APIResponse invoke(APIRequest request) {
        ...
        switch (request.getMethodName()) {
            ...
            default:
                // 404 - response null.
                responseCode = 404;
                responseMessage = "GlobalizationBridge does not provide the function '" + request.getMethodName() + "' Please check your client-side API version; should be API version >= v2.0.8.";
        }
