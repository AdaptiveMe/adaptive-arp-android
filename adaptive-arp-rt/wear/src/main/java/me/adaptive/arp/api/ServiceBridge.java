...
public class ServiceBridge extends BaseCommunicationBridge implements IService, APIBridge {
    ...
    public ServiceRequest getServiceRequest(ServiceToken serviceToken) {
        ...
        if (logger != null)
            logger.log(ILoggingLogLevel.DEBUG, this.apiGroup.name(), this.getClass().getSimpleName() + " executing getServiceRequest({" + serviceToken + "}).");
    ...
    public ServiceToken getServiceToken(String serviceName, String endpointName, String functionName, IServiceMethod method) {
        ...
        if (logger != null)
            logger.log(ILoggingLogLevel.DEBUG, this.apiGroup.name(), this.getClass().getSimpleName() + " executing getServiceToken({" + serviceName + "},{" + endpointName + "},{" + functionName + "},{" + method + "}).");
    ...
    public void invokeService(ServiceRequest serviceRequest, IServiceResultCallback callback) {
        ...
        if (logger != null)
            logger.log(ILoggingLogLevel.DEBUG, this.apiGroup.name(), this.getClass().getSimpleName() + " executing invokeService({" + serviceRequest + "},{" + callback + "}).");
    ...
    public boolean isServiceRegistered(String serviceName, String endpointName, String functionName, IServiceMethod method) {
        ...
        if (logger != null)
            logger.log(ILoggingLogLevel.DEBUG, this.apiGroup.name(), this.getClass().getSimpleName() + " executing isServiceRegistered({" + serviceName + "},{" + endpointName + "},{" + functionName + "},{" + method + "}).");
    ...
    public APIResponse invoke(APIRequest request) {
        ...
        switch (request.getMethodName()) {
            ...
            default:
                // 404 - response null.
                responseCode = 404;
                responseMessage = "ServiceBridge does not provide the function '" + request.getMethodName() + "' Please check your client-side API version; should be API version >= v2.0.8.";
        }
